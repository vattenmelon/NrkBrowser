    ...
    public class NrkParser
    {
        ...
        public List<Item> GetMestSette(int dager)
        {
            ...
            foreach (Match x in matches)
            {
                ...
                c.Description = String.Format(NrkTranslatableStrings.DESCRIPTION_CLIP_SHOWN_TIMES, c.AntallGangerVist);
                c.Bilde = x.Groups[1].Value;
        ...
        public List<Item> GetTopTabber()
        {
            ...
            foreach (Match x in matches)
            {
                ...
                item.Description = string.Format(NrkTranslatableStrings.MENU_ITEM_DESCRIPTION_NEWEST_CLIPS_FROM_GENERIC, title);
                items.Add(item);
            }
        ...
        public List<Item> GetTopTabContent(String tab)
        {
            ...
            foreach (Match x in matches)
            {
                ...
                try
                {
                    c.Description = string.Format(NrkTranslatableStrings.DESCRIPTION_CLIP_ADDED, NrkUtils.parseKlokkeSlettFraBilde(bildeUrl));
                }
                catch(Exception e)
                {
